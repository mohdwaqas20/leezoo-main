...
export default function BulkPrinting() {
  return (
    <section id="bulk-printing" style={{ background: 'var(--sand)', color: '#2a1f14', padding: '0 clamp(1rem,4vw,4rem) clamp(3rem,6vw,7rem)', borderTop: '2px solid rgba(122,87,64,0.25)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '4rem', alignItems: 'center', marginBottom: '5rem' }}>
        <div>
          <h2 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(3rem,7vw,6rem)', lineHeight: 1.0, letterSpacing: '0.04em', marginBottom: '2rem', color: '#2a1f14' }}>
            Scale Your<br />
        ...
        {/* Pricing card — bigger and more prominent */}
        <div style={{
          border: '1.5px solid var(--brown)', padding: '3.8rem 3.2rem',
          background: 'rgba(122,92,63,0.06)', minWidth: 300, textAlign: 'center',
          transition: 'border-color 0.3s, box-shadow 0.3s', display: 'flex',
          flexDirection: 'column', alignItems: 'center', gap: '0',
        }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = '#2a1f14'; e.currentTarget.style.boxShadow = '0 12px 40px rgba(42,31,20,0.14)'; }}
          onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--brown)'; e.currentTarget.style.boxShadow = 'none'; }}
        >
          ...
          <a
            ...
            style={{
              ...
              fontWeight: 600, transition: 'background 0.25s, transform 0.2s',
            }}
      ...
      <div style={{ marginBottom: '5rem' }}>
        {/* "Who We Serve" label — dark readable */}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '1rem' }}>
          {audiences.map((a) => (
            <div key={a.label} style={{
              border: '1px solid rgba(122,92,63,0.3)', padding: '2rem 1.2rem',
              textAlign: 'center', transition: 'border-color 0.3s, transform 0.3s',
              cursor: 'default',
            }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--brown)'; e.currentTarget.style.transform = 'translateY(-4px)'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(122,92,63,0.3)'; e.currentTarget.style.transform = 'translateY(0)'; }}
            >
      ...
      {/* CTA Banner */}
      <div style={{ background: '#2a1f14', padding: '2.5rem 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem' }}>
        <div>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          {/* WhatsApp — brand brown, not green */}
          <a
            href={`https://wa.me/${WA_NUMBER}?text=${WA_MSG_BULK}`}
            target="_blank" rel="noreferrer"
            style={{
          ...
          <a
            ...
            style={{
              ...
              padding: '0.9rem 1.8rem', fontSize: '0.62rem', letterSpacing: '0.22em',
              textTransform: 'uppercase', fontFamily: 'Jost,sans-serif',
              textDecoration: 'none', cursor: 'pointer', transition: 'border-color 0.3s, transform 0.2s',
            }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = '#f5ede3'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(245,237,227,0.5)'; e.currentTarget.style.transform = 'translateY(0)'; }}
          >
      ...
      <style>{`
        @media (max-width: 1000px) {
          #bulk-printing > div:first-child { grid-template-columns: 1fr !important; }
          #bulk-printing .audiences-grid { grid-template-columns: repeat(3,1fr) !important; }
        }
        @media (max-width: 600px) {
          #bulk-printing { padding: 4rem 1.5rem !important; }
          #bulk-printing .audiences-grid { grid-template-columns: repeat(2,1fr) !important; }
          #bulk-printing .cta-banner { flex-direction: column !important; }
        }
      `}</style>
    </section>
  );
